[C_PE_E,k_PE_E,eta_PE_E,eigva_PE_E,eigve_PE_E]=covari(E_PE);
plot(1-cumsum(diag(eigva_PE_E))/sum(diag(eigva_PE_E)))
polyfit([50 100 200],[k_PE_E50 k_PE_E100,k_PE_E],2)
plot(x,ans(1)*x.^2+ans(2)*x+ans(3))
hold on
plot([50 100 200],[k_PE_E50 k_PE_E100,k_PE_E])
phi=abs(sin(pi*tau)/p)/L
...
err_PE_E = 1-cumsum(diag(eigva_PE_E))/trace(eigva_PE_E));
n=1
