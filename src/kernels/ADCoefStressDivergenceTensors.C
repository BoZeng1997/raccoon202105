...
#include "ADCoefStressDivergenceTensors.h"
#include "RankTwoTensor.h"
#include "libmesh/quadrature.h"

registerMooseObject("raccoonApp", ADCoefStressDivergenceTensors);

InputParameters
ADCoefStressDivergenceTensors::validParams()
{
  InputParameters params = ADKernel::validParams();
  params.addClassDescription("Stress divergence kernel with automatic differentiation for the "
                             "Cartesian coordinate system with quadratic degradation function");
  ...
  params.addCoupledVar("d","phase-field damage");
  return params;
}

ADCoefStressDivergenceTensors::ADCoefStressDivergenceTensors(const InputParameters & parameters)
  : ADKernel(parameters),
...
void
ADCoefStressDivergenceTensors::initialSetup()
{
...
ADReal
ADCoefStressDivergenceTensors::computeQpResidual()
{
  ADReal residual = (1.0-_d[_qp])*(1.0-_d[_qp])*_stress[_qp].row(_component) * _grad_test[_i][_qp];
...
void
ADCoefStressDivergenceTensors::precalculateResidual()
{
  if (!_volumetric_locking_correction)
    return;
